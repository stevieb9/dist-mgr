...
I<Options>:
    ...
    -k | --buildcheck Optional: Insert the RPi::Const::BuildCheck guard into
                                Makefile.PL (RPi:: family only). Takes a type:
                                'wiringpi' or 'i2c'
...
All of the below options are flags (they take no arguments), except
C<--buildcheck> which takes a type argument.
    ...
    -k | --buildcheck   Insert the RPi::Const::BuildCheck guard into
                        Makefile.PL. Takes a type: 'wiringpi' or 'i2c'
...
I<Note>: C<--buildcheck> is RPi-specific and deliberately excluded from
C<--all>; supply it explicitly (eg. C<--buildcheck wiringpi>) only for
C<RPi::> family distributions.
