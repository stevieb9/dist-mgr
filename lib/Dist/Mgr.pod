...
I<Optional, Hash Reference>: See L<CPAN::Uploader> for full details. A couple of
notes:
...
Using C<system()>, perform a C<make dist> that creates a CPAN-ready distribution
tarball file in the current working directory.

I<Use>: After all other pre-release functions have been called.

I<Returns>: C<0> on success. Anything other than C<0> is a failure.

=head2 make_test

Using C<system()>, perform a C<%^X Makefile.PL> and C<make test>.
