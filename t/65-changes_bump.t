...
# version_bump()
{
    ...
    if ($^O =~ /win/i) {
        my $actual_result = 'RevisionhistoryforTest-Module5.55UNREL-0.012020-12-26-AutogenerateddistributionwithDist::Mgr';
        my $stringified_result = file_scalar($work);
        $stringified_result =~ s/[\n\s]//g;
        is $stringified_result, $actual_result, "on Windows, results are ok";
    }
    else {
